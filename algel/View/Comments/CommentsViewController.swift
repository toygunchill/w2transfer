...
extension CommentsViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        commentCell.layer.applySketchShadow(color: UIColor.black,
                                            alpha: 0.1,
                                            x: 2,
                                            y: 2,
                                            blur: 10,
                                            spread: 0)
        if let rating = commentList?[indexPath.item].rating {
            commentCell.commentScoreLbl.text = rating
        }
        if let comment = commentList?[indexPath.item].comment {
            commentCell.commentInfoLbl.text = comment
        }
        if let price = commentOtherDataList?[indexPath.item].price {
            commentCell.commentAmountLbl.text = "\(price) TL"
        }
