...
class HomeViewController: UIViewController {
    ...
    override func viewDidLoad() {
       ...
       // createOrderViewController?.delegate = self
        ...
        configureInfoView(visible: false) // true çağrılacak
        configureMapView()
        mapView.showsUserLocation = true
        let tapCenterGesture = UITapGestureRecognizer(target: self, action: #selector(centerBtnIVTapped))
        configureTouchableIV(iv: centerBtnIV,
                             gestrue: tapCenterGesture)
        let tapCreateGesture = UITapGestureRecognizer(target: self, action: #selector(createBtnIVTapped))
        configureTouchableIV(iv: createOrderBtn,
                             gestrue: tapCreateGesture)
        let tapOrderGesture = UITapGestureRecognizer(target: self, action: #selector(orderBtnIVTapped))
        configureTouchableIV(iv: orderIV,
                             gestrue: tapOrderGesture)
        let tapOwnerIVGesture = UITapGestureRecognizer(target: self, action: #selector(ownerIVTapped))
        configureTouchableIV(iv: ownerIV, gestrue: tapOwnerIVGesture)
    ...
    func configureInfoView(visible: Bool) {
        if visible == false {
    ...
    @objc func orderBtnIVTapped() {
        if let activeVC = storyboard?.instantiateViewController(withIdentifier: "ActiveOrderViewController") as? ActiveOrderViewController {
            if orderIV.image == UIImage(systemName: "1.lane") {
                GlobalService.sharedGlobal.activeOrder = activeOrder
    ...
    @IBAction func ownerProductAcceptBtnTapped(_ sender: Any) {
        if activeCount == 0 {
            ...
            viewModel.postOrderStateUpdate(orderID: activeOrder?.id, active: "2") { result in
                if result == "true" {
...
extension HomeViewController: MKMapViewDelegate, CLLocationManagerDelegate {
    ...
    func mapView(_ mapView: MKMapView, didSelect view: MKAnnotationView) {
        if let annotation = view.annotation {
            ...
            print("Tıklanan pin: \(annotation.title ?? "") - Koordinatlar: \(coordinateString)")
            orderList?.forEach({ order in
                if order.longitude == "\(annotation.coordinate.longitude)",
                   order.latitude == "\(annotation.coordinate.latitude)" {
                    activeOrder = order
                    ownerInfoView.isHidden = false
                    ownerProductOrderRoadLbl.text = "Road = \(order.road)"
                    ownerProductWightLbl.text = "Weight = \(order.type)"
                    ownerProductNoteLbl.text = "Note = \(order.note)"
                    ownerProductOrderOfferLbl.text = "Price = \(order.price)"
                    ownerProductLocationLbl.text = "Location = \(order.pickupAdress) - \(order.dropAdress)"
                }
            })
        }
    }
